...
const loading=ref(false);const error=ref('');const logs=ref<AuditLog[]>([]);const action=ref('')
async function load(){loading.value=true;error.value='';try{logs.value=(await getAuditLogs(action.value?{action:action.value}:undefined)).data.items}catch(e:any){error.value=e?.response?.data?.message||'审计日志加载失败'}finally{loading.value=false}}
