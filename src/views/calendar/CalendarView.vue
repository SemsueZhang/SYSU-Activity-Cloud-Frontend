  ...
  <AppShell title="我的日历">
    ...
    <PageState :loading="loading" :error="error" @retry="load">
      <div class="calendar-layout">
        ...
        <aside class="surface-card event-panel"><h2>{{ selected }} 日程</h2><el-empty v-if="!selectedEvents.length" description="当日暂无日程"/><template v-else><button v-for="event in selectedEvents" :key="event.id || event.title" type="button" class="event event-link" :aria-label="`查看活动：${event.title}`" @click="router.push(`/activity/${event.activity_id}`)"><time>{{ event.time }}</time><div><strong>{{ event.title }}</strong><p>报名活动</p></div></button></template></aside>
      </div>
    </PageState>
  </AppShell>
</template>
