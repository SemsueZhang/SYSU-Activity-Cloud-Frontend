...
\item \textbf{接口隔离：} 前端通过 \texttt{api/} 模块封装所有 HTTP 请求，页面组件不直接调用 Axios 或处理 JSON 序列化，接口变更时只需修改对应的 API 封装层。后端服务层与接口层通过服务函数签名解耦。
...
\textbf{前端分层（自顶向下，下层为上层提供服务）：}
\begin{enumerate}
\item \textbf{视图层：} 页面级组件（views），每个视图对应一个路由入口，负责页面组装和状态消费。
\item \textbf{组件层：} 可复用 UI 组件（components），包含通用布局（AppShell、NavHeader）和业务组件（CaptchaField），消费 store 中的状态。
\item \textbf{状态管理层：} Pinia store（stores/），管理跨页面的认证状态和共享数据，调用 API 层获取数据。
\item \textbf{API 封装层：} Axios 请求封装（api/），负责 HTTP 调用、响应数据适配和字段重命名，被 store 和组件调用。
\item \textbf{工具层：} 纯函数和组合式逻辑（utils/、composables/），放置排序、质量评分等可独立测试的业务逻辑，不依赖任何其他前端层。
\end{enumerate}

\textbf{后端分层（自顶向下）：}
\begin{enumerate}
\item \textbf{接口层：} Flask 蓝图，负责 HTTP 路由、参数解析、JWT 校验和响应序列化。
...
\multicolumn{3}{c}{\textbf{前端层}} \\\cmidrule(lr){1-3}
视图层 & 页面组装、路由分发、状态消费 & views、router、stores \\
组件层 & 可复用 UI 组件、通用布局 & AppShell、NavHeader、PageState \\
状态管理层 & 跨页面状态管理、数据获取 & Pinia stores（auth 等） \\
API 封装层 & HTTP 请求、数据适配 & \texttt{api/} 模块 \\
工具层 & 纯函数、组合式逻辑 & utils/、composables/ \\
\multicolumn{3}{c}{\textbf{后端层}} \\\cmidrule(lr){1-3}
接口层 & HTTP 路由、参数解析、JWT 校验 & auth、activities 等蓝图 \\
服务层 & 业务规则、质量、去重、审计、通知 & \texttt{services/*} \\
持久层 & 实体定义、关系映射、约束、事务 & Poster、User、Registration 等 \\
基础设施层 & 数据库、缓存、队列、外部服务 & extensions、config、Docker Compose \\
\bottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{p{2.6cm}p{5.4cm}Y}
...
\item \textbf{统一响应结构：} 非分页的成功响应直接返回数据体（如 \texttt{\{"id": 1, "title": "..."\}}）；分页的成功响应返回 \texttt{\{items: [...], page, per\_page, total\}}，默认每页 20 条。错误响应统一为 \texttt{\{"error": "描述信息"\}} 格式；多字段校验错误返回 \texttt{\{"error": "描述", "field\_errors": \{"title": "标题不能为空"\}\}}。HTTP 状态码与语义一致（400 参数错误、401 未认证、403 越权、404 资源不存在、500 服务端错误）。
...
\item \textbf{搜索服务：} 提供全文检索能力。内部搜索基于数据库 LIKE 查询为默认实现（注意：前导通配符 \texttt{\%keyword\%} 无法利用数据库索引，千级数据量下性能可接受，万级以上建议启用外部搜索引擎）；外部搜索引擎（如 Elasticsearch）为可选增强；外部搜索超时 10s，超时后回退到内部搜索。
\item \textbf{AI 服务：} 用于活动摘要生成和内容推荐。通过 REST API 调用 LLM 服务，超时 30s，超时或不可用时返回基础摘要或默认推荐列表，不影响活动浏览和审核核心路径。
\end{enumerate}

\section{关键设计决策}
\subsection{技术选型}
\begin{longtable}{p{2.4cm}p{3.2cm}p{3.8cm}p{3.8cm}}
\toprule 决策 & 选型方案 & 理由 & 替代方案 \\\midrule\endhead
前端框架 & Vue 3 + TypeScript & 类型安全、组合式 API 逻辑复用、社区活跃 & React、Angular \\
状态管理 & Pinia & 官方推荐、TypeScript 友好、API 简洁 & Vuex、reactive composables \\
后端框架 & Flask + 蓝图 & 轻量灵活、生态成熟、学习成本低 & Django、FastAPI \\
ORM & SQLAlchemy & 支持多数据库切换、迁移工具链成熟 & Peewee、Django ORM \\
认证 & JWT + RBAC & 无状态、跨域友好、角色边界明确 & Session/Cookie、OAuth \\
容器化 & Docker + Compose & 环境可复现、单命令部署 & Kubernetes、手动部署 \\
\bottomrule
\end{longtable}
...
\subsection{方案权衡}
\noindent\begin{tabularx}{\textwidth}{p{1.5cm}p{3.2cm}p{4.5cm}p{4.5cm}}
...
\subsection{可维护性}
\begin{enumerate}
\item \textbf{分层隔离：} 前端按 views / components / stores / api / composables+utils 分层，后端按接口 / 服务 / 持久层分层，每层职责明确，下层为上层提供服务，修改范围可控。
