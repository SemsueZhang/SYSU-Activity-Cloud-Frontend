...
| 个人日历 | `GET /calendar/events`、`/posters/{id}/ics` | 已接入 | 日历只展示报名同步产生的活动和个人日程，不再提供活动的独立加入/移除操作；仍支持 ICS 导出。 |
...
| 公共活动列表、详情、创建编辑 | 当前 Mock：`/activities`；后端文档：`/posters` | 待后端确认适配 | 前端路由固定为 `/activities` 和 `/activity/:id`；需要后端确认列表权限与“我创建的活动”查询参数后，再在 API 适配层统一切换资源路径。 |
| 报名与独立收藏 | 当前 Mock：`POST/DELETE /activities/{id}/register`、`/favorite` | 待接口冻结 | Mock 将报名与日历写入视为一个原子操作，取消报名同步删除日历事件；真实后端需提供同等一致性，前端不再单独调用日历写接口。 |
